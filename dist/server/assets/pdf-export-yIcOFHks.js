...
function buildReportTableHtml({
  ...
  const bodyRows = rows.length > 0 ? rows.map(
    (r, i) => `<tr style="background:${i % 2 ? "#f8fafc" : "#ffffff"}">` + r.map(
      (cell, ci) => `<td style="padding:6px 8px;border-bottom:1px solid #e5e7eb;${ci === 0 ? "" : "text-align:end;font-variant-numeric:tabular-nums;"}">${escapeHtml(cell)}</td>`
    ).join("") + `</tr>`
  ).join("") : `<tr><td colspan="${headers.length}" style="padding:18px;text-align:center;color:#94a3b8;">—</td></tr>`;
  return `
      ...
      <table style="width:100%;border-collapse:collapse;font-size:12px;direction:${dirAttr};">
        <thead>
          <tr style="background:#0f172a;color:#ffffff;">
            ${headers.map(
    (h, i) => `<th style="padding:8px;text-align:${i === 0 ? "start" : "end"};">${escapeHtml(h)}</th>`
  ).join("")}
          </tr>
        </thead>
        <tbody>${bodyRows}</tbody>
      </table>
      ${footerText ? `<div style="margin-top:18px;font-size:10px;color:#94a3b8;text-align:center;">${escapeHtml(footerText)}</div>` : ""}
    </div>`;
}
export {
